# Kubeadm cluster with docker as CRI:
...
Notes:
total 3 modes. enforcing, permissive, disabled. 
permissive mode: selinux policies are not enforced. but logged.
...
IP: Get the MAC address of the network interfaces using 'ifconfig' or 'ip a' commands:
```
ip a
```
...
# Required CNI UDP Port (for Flannel), # to allow worker to worker traffic
```

```
sudo firewall-cmd --permanent --add-port=30000-32767/tcp 
# for 'NodePort service' exposure (Needed if the master will also run user workloads).
```
I think it's optional. need to verify. #remove 

```
firewall-cmd --permanent --zone=public --add-source=10.244.0.0/16 

# to allow pod network interface
```
...
# for k8s - NodePort Services exposure
...
- 10257 (controller manager) and 10259 (scheduler). These ports are internal communication ports and are not strictly necessary to open in the firewall. 
- since communication happens over the internal network interfaces (which are usually less restricted) or via the API server. 
- So, Keeping only 6443, 2379-2380, 10250, and 8472 is sufficient and cleaner on master node. 
- And for worker node, 10250, 8472, 30000-32767. But, make sure to add public zone with pod network NIC IP.
...
## STAGE 2: 
Install CRI (docker) & K8s-components: kubeadm, kubelet, kubectl (on all nodes)


8. Steps to install 'docker-engine'. (on all nodes)
...
container-selinux, fuse-common, fuse-overlayfs, fuse3, iptables-nft, passt, passt-selinux, tar, docker-ce-rootless-extras (weak dependency)
...
8.6: Add the os user to the docker group (to run the docker cli commands as non root user)
as os user, run:
```
...
on official github page, for version v0.3.12, available packages for el8 are:
...
Note:
Without libcgroup, I am getting an error while installing 'cri-dockerd'
...
Recommended method. 
Always pickup the latest release, but not pre-release version. (version: 'v0.3.20')

Installing necessary utility: wget, tar. 
Use, `which wget/tar` or `dnf list installed wget/tar` commands to verify.
```
sudo dnf install -y wget tar
```

version v0.3.21 is available aswell, 
https://github.com/Mirantis/cri-dockerd/releases/download/v0.3.21/cri-dockerd-0.3.21.amd64.tgz

But I stick with version 'v0.3.20' for now:
...
Verify the whether the service is running or not:
```
...
to refetch the generated token value from 'kubeadm init' command: 
```
...
get the resource usage data with top command.
```
kubectl top nodes
```
...
To install Chrony service & enable it:
```
sudo dnf install -y chrony
sudo systemctl enable --now chronyd
```

To force an immediate time synchronization:
```
...
re-ran the above command w.r.t output message:
