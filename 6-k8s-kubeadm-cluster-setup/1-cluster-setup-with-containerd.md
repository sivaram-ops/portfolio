...
ip a
UUID: (on RHEL distros), The product_uuid can be checked by using the command: 
```
...
- So, Keeping only 6443, 2379-2380, 10250, and 8472 (if using flannel cni) is sufficient and cleaner on master node. 
...
These steps prepare the host's linux kernel for Kubernetes networking.
...
### STAGE 2: 
Install CRI (containerd) & K8s-components: kubeadm, kubelet, kubectl (on all nodes)


8. Steps to install 'containerd': (on all nodes)
...
change 'SystemdCgroup' value from 'false' to 'true' in '/etc/containerd/config.toml' file.
In this file, 'SystemCgroup' value is located in this section: [plugins."io.containerd.grpc.v1.cri".containerd.runtimes.runc.options]

Note: #remove 
Along with the 'SystemdCgroup', we might find another value, as 'system=cgroup'. Don't change that. I breaked my cluster by changing it.
...
create a repository (kubernetes.repo) to install kubelet, kubeadm, kubectl.
```
...
- Initialize a cluster with '--pod-network-cidr'. 
- For single master node, '--control-plane-endpoint' flag is optional.
...
(or) as a root user, run:
```
export KUBECONFIG=/etc/kubernetes/admin.conf
```


2. deploy a 'pod network add-on'. (on master node)
